...
def triangleMotion(velocity_publisher, side_length):
    while True:
        try:
            side_length = float(input("Enter the side length of the triangle (in meters): "))
            if side_length > 5.5:
                print("Invalid side length! The side length must be 10.5 meters or less to stay within the grid.")
            elif side_length <= 0:
                print("Invalid side length! The side length must be greater than 0.")
...
def point_to_point(velocity_publisher):
       while True:
        try:
            x_goal = float(input("Enter the X-coordinate of the target point (0 to 10.5): "))
            y_goal = float(input("Enter the Y-coordinate of the target point (0 to 10.5): "))

            # Validate that the coordinates are within the boundaries of the grid (0, 0) to (10.5, 10.5)
            if x_goal >0 or x_goal < 5.5 or y_goal > 0 or y_goal <5.5:
                print("Invalid coordinates! Both X and Y coordinates must be between 0 and 10.5.")
            else:
                break  # Valid input, exit the loop
        except ValueError:
...
def zigzag(velocity_publisher,side_length, count):
   while True:
        try:
            side_length = float(input("Enter the side length of the square (in meters): "))
            if side_length > 5:
                print("Invalid side length! The side length must be 5 meters or less to stay within the grid.")
            elif side_length <= 0:
                print("Invalid side length! The side length must be greater than 0.")
            else:
                break  # Valid input, exit the loop
                ...
                if choice == 1:
                    ...
                    try:
                        ...
                        if raduis <= 0:
                            ...
                            circularMotion(raduis)  # Pass the radius to the function
                    except ValueError:
                    ...
                    point_to_point(velocity_publisher)
                elif choice == 6:
